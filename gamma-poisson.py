...
fit = StanFit(code, data)  # use code string in this script file
...
plot(fit.alpha.samples, log10(fit.flux_cut.samples), 'b.', alpha=.4)
...
hist(fit.alpha.samples, 20, alpha=.4)
axvline(alpha, **xhair)
xlabel(r'$\alpha$')
ylabel(r'$p(\alpha|D)$')

subplot(224)  # marginal for F_cut bottom-right
hist(log10(fit.flux_cut.samples), 20, alpha=.4)
...
i = fit.log_p.samples.argmax()
a, F = fit.alpha.samples[i], fit.flux_cut.samples[i]
